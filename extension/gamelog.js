...
(function () {
  ...
  function gameLogInfo() {}
  function gameLogWarn() {}
  function gameLogError() {}
    ...
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.runtime && chrome.runtime.id) {
      ...
      setTimeout(() => {
        if (!storageCallbackFired) {
          gameLogWarn('[TM-Log] Storage callback timed out — starting fresh log');
          logReady = true;
        }
      }, 3000);
      ...
      try {
        chrome.storage.local.get('gamelog_' + gameId, (data) => {
          ...
          if (chrome.runtime.lastError) {
            gameLogWarn('[TM-Log] Storage read error:', chrome.runtime.lastError.message);
            logReady = true;
            return;
          }
        ...
        gameLogWarn('[TM-Log] Storage access failed:', e.message);
        logReady = true;
      }
    } else {
      logReady = true;
  ...
  function _writeFinalState(log, bridgeData, gen) {
    ...
    } catch(e) { gameLogWarn('[TM-Log] draft reconstruction failed:', e); }

    try { autoExportLog(log, gen); } catch(e) { gameLogWarn('[TM-Log] auto-export failed:', e); }
  }
  ...
  function discoverOpponents(tmGameId) {
    ...
    fetch('/api/game?id=' + encodeURIComponent(tmGameId))
      ...
      .then(function(data) {
        ...
        gameLogInfo('[TM-Log] Opponent draft tracking: ' + Object.keys(_oppPlayers).length + ' players');
        scheduleOppPoll();
      })
      .catch(function(e) {
        gameLogWarn('[TM-Log] Opponent discovery failed:', e.message);
  ...
  function autoExportLog(log, gen) {
    ...
    } catch(e) { gameLogWarn('[TM-Log] text export failed:', e); }

    gameLogInfo('[TM-Log] Auto-exported game log (' + log.events.length + ' events)');
  }
  ...
  function saveLog() {
    ...
    safeStorage((storage) => {
      ...
      storage.local.set(data, () => {
        try {
          if (chrome.runtime.lastError) {
            gameLogWarn('[TM-Log] Save failed:', chrome.runtime.lastError.message);
            if (chrome.runtime.lastError.message.indexOf('quota') >= 0 || chrome.runtime.lastError.message.indexOf('QUOTA') >= 0) {
              storage.local.get(null, (all) => {
                ...
                if (keys.length > 0) {
                  gameLogWarn('[TM-Log] Purging', keys.length, 'old gamelogs to free quota');
