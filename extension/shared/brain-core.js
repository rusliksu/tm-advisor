...
;(function(root) {
  ...
  function findNamedCard(cards, name) {
    cards = cards || [];
    for (var i = 0; i < cards.length; i++) {
      if (cardNameOf(cards[i]) === name) return cards[i];
    }
    return null;
  }
  ...
  function coloniesFromState(state) {
    ...
    return asArray(game.colonies || state && state.colonies || game.coloniesModel || state && state.coloniesModel);
  }
  ...
  function hasTableauCard(state, name) {
    ...
    return !!findNamedCard(tableau, name);
  }
  ...
  function analyzeMinorityRefugeMirandaSequence(options) {
    var opts = options || {};
    var state = opts.state || {};
    var cards = asArray(opts.cards || (state && state.cardsInHand) || (state && state.thisPlayer && state.thisPlayer.cardsInHand));
