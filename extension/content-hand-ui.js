...
(function(global) {
  ...
  function cardTagsContain(card, tagName) {
    var tags = toTagArray(card && card.tags);
    var target = String(tagName || '').toLowerCase();
  ...
  function isFinalCashoutWindow(input, pv) {
    var gensLeft = getGensLeft(input, pv);
    if (gensLeft != null) return gensLeft <= 1;
    var gen = Number(pv && pv.game && pv.game.generation);
    return isFinite(gen) && gen >= 9;
  }

  function countCitiesInPlay(pv) {
    var players = Array.isArray(pv && pv.players)
      ? pv.players
      : (Array.isArray(pv && pv.game && pv.game.players) ? pv.game.players : []);
    var count = 0;
    for (var i = 0; i < players.length; i++) {
      count += Math.max(0, Number(players[i] && players[i].citiesCount) || 0);
    }
    var boardCount = 0;
    var spaces = pv && pv.game && pv.game.spaces;
  ...
  function hasCorpName(pv, input, corpName) {
    var target = String(corpName || '').toLowerCase();
    var corps = typeof input.detectMyCorps === 'function' ? input.detectMyCorps() : [];
    if (Array.isArray(corps)) {
      for (var ci = 0; ci < corps.length; ci++) {
        if (String(corps[ci] || '').toLowerCase() === target) return true;
      }
    }
    var player = (pv && pv.thisPlayer) || {};
    if (String(player.corporation || player.corp || '').toLowerCase() === target) return true;
    var tableau = Array.isArray(player.tableau) ? player.tableau : [];
    for (var ti = 0; ti < tableau.length; ti++) {
      var name = tableau[ti] && (tableau[ti].name || tableau[ti]);
      if (String(name || '').toLowerCase() === target) return true;
    }
    return false;
  }

  function greenhousesDeferredReason(input, pv) {
    var cities = countCitiesInPlay(pv);
    if (cities <= 0) return '';
    var player = (pv && pv.thisPlayer) || {};
    var plants = Math.max(0, Number(player.plants) || 0);
    var plantCost = hasCorpName(pv, input || {}, 'Ecoline') ? 7 : 8;
    var before = Math.floor(plants / plantCost);
    var after = Math.floor((plants + cities) / plantCost);
    if (after > before) return '';
    var gensLeft = getGensLeft(input, pv);
    if (gensLeft != null && gensLeft <= 1) return '';
    return 'late plant cashout; ' + cities + ' cities now, ' + plants + '->' + (plants + cities) + ' plants';
  }
  ...
  function countSpaceEventTriggers(cards, selfName) {
    ...
    for (var i = 0; i < cards.length; i++) {
      var card = cards[i] || {};
      if (!card.name || card.name === selfName) continue;
      if (cardTagsContain(card, 'space') && cardTagsContain(card, 'event')) count++;
    }
    return count;
  }

  function triggerSetupDeferredReason(name, input, pv, cards) {
    if (name !== 'Optimal Aerobraking') return '';
    var triggers = countSpaceEventTriggers(cards, name);
    var gensLeft = getGensLeft(input, pv);
    if (triggers >= 2) return '';
    if (triggers >= 1 && gensLeft != null && gensLeft <= 1) return '';
    if (triggers >= 1) return 'setup card; play right before the space event';
    return 'setup card; no space event trigger in hand';
  }

  function deferredPlayReason(name, input, pv, cards) {
    if (name === 'Greenhouses') return greenhousesDeferredReason(input || {}, pv);
    var triggerReason = triggerSetupDeferredReason(name, input || {}, pv, cards);
    if (triggerReason) return triggerReason;
    if (!name || !DEFERRED_SCALING_CASHOUTS[name]) return '';
    if (isFinalCashoutWindow(input, pv)) return '';
  ...
  function hasAdvancedAlloysInHand(cards) {
    if (!cards) return false;
    for (var i = 0; i < cards.length; i++) {
      if (cards[i] && cards[i].name === 'Advanced Alloys') return true;
    }
    return false;
  }

  function handCardByName(cards, name) {
    if (!cards) return null;
    for (var i = 0; i < cards.length; i++) {
      if (cards[i] && cards[i].name === name) return cards[i];
    }
    return null;
  }

  function advancedAlloysPlayable(cards, pv, input) {
    var alloys = handCardByName(cards, 'Advanced Alloys');
    if (!alloys) return false;
    return buyingPower(alloys, pv) >= effectiveCost(alloys, input || {});
  }

  function metalSpendTargetForAdvancedAlloys(card, pv) {
    if (!card || card.name === 'Advanced Alloys') return false;
    var player = (pv && pv.thisPlayer) || {};
    if (cardTagsContain(card, 'building') && Math.max(0, Number(player.steel) || 0) > 0) return true;
    if (cardTagsContain(card, 'space') && Math.max(0, Number(player.titanium) || 0) > 0) return true;
    return false;
  }

  function hasAdvancedAlloysMetalSpendTarget(cards, pv) {
    if (!cards) return false;
    for (var i = 0; i < cards.length; i++) {
      if (metalSpendTargetForAdvancedAlloys(cards[i], pv)) return true;
    }
    return false;
  }

  function playOrderAdjustment(name, pv, cards, input) {
    if (!hasAdvancedAlloysInHand(cards) || !advancedAlloysPlayable(cards, pv, input) || !hasAdvancedAlloysMetalSpendTarget(cards, pv)) {
      return { adj: 0, reason: '' };
    }
    if (name === 'Advanced Alloys') {
      return { adj: 18, reason: 'play before steel/titanium spend' };
    }
    for (var i = 0; i < cards.length; i++) {
      if (cards[i] && cards[i].name === name && metalSpendTargetForAdvancedAlloys(cards[i], pv)) {
        return { adj: -12, reason: 'after Advanced Alloys' };
      }
    }
    return { adj: 0, reason: '' };
  }
    ...
    if (cardName && typeof input.getProductionFloorStatus === 'function') {
      ...
      if (prodFloorStatus && prodFloorStatus.unplayable) {
        return compactText(prodFloorStatus.reasons && prodFloorStatus.reasons[0])
          .replace('Невозможно сыграть: ', 'Не сейчас: ');
      }
    }
  ...
  function injectHandPriorityBadges(input) {
    ...
    for (var ri = 0; ri < ranked.length; ri++) {
      ...
      var orderAdjustment = playOrderAdjustment(rankedName, pv, cards, input);
      displayRows.push({
        row: rankedRow,
        deferredReason: deferReason,
        orderReason: orderAdjustment.reason,
        adjustedScore: (originalScore == null ? 0 : originalScore) - (deferReason ? 45 : 0) + (orderAdjustment.adj || 0),
        originalIndex: ri
      });
    }
