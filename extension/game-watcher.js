...
(function() {
  ...
  async function discoverPlayers() {
    try {
      ...
      gwWarn('[TM Watcher] Failed to discover players:', e.message);
      return false;
    }
  }
  ...
  async function pollAll() {
    ...
    try {
      ...
      if (failCount === playerIds.length) {
        consecutiveFailures++;
        if (consecutiveFailures === MAX_CONSECUTIVE_FAILURES) {
          gwWarn('[TM Watcher] All polls failing — switching to backoff interval');
        }
      } else {
      ...
      if (state.gamePhase === 'end' && !state.gameEnded) {
        ...
        try {
          if (localStorage.getItem(watchExportKey)) {
            gwLog('[TM Watcher] Already exported this game, skipping');
      ...
      gwWarn('[TM Watcher] Poll error:', e.message);
    }
  ...
  async function pollPlayer(playerId) {
    ...
    try {
      ...
      if (!resp.ok) {
        if (resp.status === 429) {
          gwWarn('[TM Watcher] Rate limited polling', p.name);
        } else if (resp.status >= 500) {
          gwWarn('[TM Watcher] Server error', resp.status, 'polling', p.name);
        }
        return false;
      }
      const data = await resp.json();
      ...
      gwWarn('[TM Watcher] Poll failed for', p.name + ':', e.message);
      return false;
    }
  }
      ...
      chrome.storage.local.set(data, () => {
        try {
          if (chrome.runtime.lastError) {
            gwWarn('[TM Watcher] Save failed:', chrome.runtime.lastError.message);
          }
        } catch(e) { /* extension context invalidated */ }
      });
    } catch (e) {
      gwWarn('[TM Watcher] Storage write failed:', e.message);
    }
  }
  ...
  async function loadState() {
    ...
    return new Promise((resolve) => {
      ...
      chrome.storage.local.get(STORAGE_KEY, (result) => {
        try {
          if (chrome.runtime.lastError) {
            gwWarn('[TM Watcher] Load state failed:', chrome.runtime.lastError.message);
            resolve(false);
            return;
          }
        ...
        gwWarn('[TM Watcher] Storage read failed:', e.message);
        resolve(false);
      }
    });
  }
    ...
    if (!restored || Object.keys(state.players).length === 0) {
      ...
      if (!found) {
        if (initAttempts >= MAX_INIT_ATTEMPTS) {
          gwError('[TM Watcher] Failed to discover players after', MAX_INIT_ATTEMPTS, 'attempts. Giving up.');
          return;
        }
        gwWarn('[TM Watcher] No players found. Retrying in 5s... (attempt', initAttempts, '/', MAX_INIT_ATTEMPTS + ')');
