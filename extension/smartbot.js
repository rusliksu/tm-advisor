...
let GAME_ID = 'ga7375200c915';
let PLAYERS = [  { name: 'Bot1', id: 'pb231185eac47' },  { name: 'Bot2', id: 'p90d7bdae90e4' },];
...
function handleInput(wf, state, depth = 0) {
  ...
  if (t === 'or') {
    ...
    // Limit: max 1 delegate per turn cycle to avoid spending all MC on delegates
    const delegatesSent = state._delegatesSentThisTurn || 0;
    if (delegateIdx >= 0 && mc >= 8 && delegatesSent < 1) {
      state._delegatesSentThisTurn = delegatesSent + 1;
  ...
  if (t === 'space') {
    ...
    const scored = spaces.map(s => {
      ...
      if (spaceData && spaceData.adjacentSpaces) {
        let emptyAdj = 0;
        let totalAdj = spaceData.adjacentSpaces.length;
        for (const adjId of spaceData.adjacentSpaces) {
          ...
          // For city: adjacent to opponent city blocks our greenery there
          if (isCity && adj.tileType === 'city' && adj.color !== myColor) score -= 1;
          ...
          // Count empty adjacent hexes (potential for future greeneries)
          if (!adj.tileType) emptyAdj++;
        }
        // City adjacency potential: empty neighbors = future greenery VP
        // Each empty neighbor ~ 0.6 chance of becoming our greenery = ~1.8 VP value
        if (isCity) {
          score += emptyAdj * 2;  // strong bonus for each empty neighbor
          // Penalty for edge placement: fewer total adjacencies = less VP ceiling
          if (totalAdj <= 3) score -= 3;  // corner hex
          else if (totalAdj <= 4) score -= 1;  // edge hex
        }
        // Greenery: prefer placing next to own cities (already scored above)
        // Also prefer empty neighbors for future expansion
        if (isGreenery) {
          score += emptyAdj * 0.5;
        }
      }
...
// Parse CLI args: --game ID --players "name1:id1,name2:id2"
for (let i = 2; i < process.argv.length; i++) {
  if (process.argv[i] === "--game" && process.argv[i+1]) {
    GAME_ID = process.argv[++i];
  } else if (process.argv[i] === "--players" && process.argv[i+1]) {
    PLAYERS = process.argv[++i].split(",").map(s => {
      const [name, id] = s.split(":");
      return { name: name.trim(), id: id.trim() };
    });
  }
}
const _cmd = process.argv.find((a, j) => j >= 2 && !a.startsWith('--') && (j < 3 || !['--game','--players'].includes(process.argv[j-1])));
if (_cmd === 'new') {
  createGame().then(g => {
    const pstr = g.players.map(p => p.name + ':' + p.id).join(',');
    console.log(`
Run: node smartbot.js --game ${g.id} --players "${pstr}"`);
