...
(function() {
  ...
  function normalizePlayerPayload(pv) {
    if (!pv || typeof pv !== 'object') return pv;
    if (pv.thisPlayer) return pv;
    if (pv.playerView && pv.playerView.thisPlayer) return pv.playerView;
    if (pv.player && pv.player.thisPlayer) return pv.player;
    if (pv.player && pv.player.game && pv.player.color && !pv.player.thisPlayer) {
      pv = pv.player;
    }
    // Legacy payloads (e.g. pe5ha): root object is the player itself.
    if (pv.game && pv.players && pv.color && !pv.thisPlayer) {
      var wrapped = {
        thisPlayer: pv,
        players: pv.players || [],
        game: pv.game || null,
        _source: pv._source || 'legacy-api'
      };
      if (pv.waitingFor) wrapped.waitingFor = pv.waitingFor;
      if (pv.waitingFor && !wrapped._waitingFor) wrapped._waitingFor = pv.waitingFor;
      if (pv.draftedCards) wrapped.draftedCards = pv.draftedCards;
      if (pv.dealtCorporationCards) wrapped.dealtCorporationCards = pv.dealtCorporationCards;
      if (pv.dealtPreludeCards) wrapped.dealtPreludeCards = pv.dealtPreludeCards;
      if (pv.pickedCorporationCard) wrapped.pickedCorporationCard = pv.pickedCorporationCard;
      if (pv.preludeCardsInHand) wrapped.preludeCardsInHand = pv.preludeCardsInHand;
      if (pv.dealtProjectCards) wrapped.dealtProjectCards = pv.dealtProjectCards;
      return wrapped;
    }
    return pv;
  }
  ...
  function findPlayerView() {
    ...
    if (pv) {
      dlog('Using Vue data');
      return normalizePlayerPayload(pv);
    }
    ...
    if (_apiData && (Date.now() - _apiTimestamp) < 30000) {
      ...
      return normalizePlayerPayload(_apiData);
    }
