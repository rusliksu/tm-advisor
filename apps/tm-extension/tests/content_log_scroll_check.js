...
function makeCardEl(name, oldBadge) {
  return {
    textContent: name,
    attrs: {},
    ...
    querySelector(selector) {
      return selector === '.tm-log-card-score' ? oldBadge || null : null;
    },
...
function testExistingLogBadgeUpdateKeepsBottomScroll() {
  const scrollable = makeScrollable(676);
  const logPanel = makeLogPanel(scrollable);
  const oldBadge = {
    textContent: 'C 50',
    className: 'tm-log-card-score tm-log-tier tm-tier-C',
    style: {},
  };
  const card = makeCardEl('Research', oldBadge);
  logUi.decorateLogCardScores({
    documentObj: makeDocument(logPanel, [card]),
    ratings: {Research: {s: 77, t: 'B'}},
  });

  assert.strictEqual(oldBadge.textContent, 'B 77');
  assert.strictEqual(oldBadge.className, 'tm-log-card-score tm-log-tier tm-tier-B');
  assert.strictEqual(scrollable.scrollTop, 1000, 'log should stick to bottom when an existing badge mutates near-bottom panel');
}
...
testExistingLogBadgeUpdateKeepsBottomScroll();
