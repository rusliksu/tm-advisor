...
def cmd_notes(args):
    """Generate/update player_notes.md from game data."""
    db = load_db()
    games = list(db["games"].values())
    if not games:
        print(f"{Fore.YELLOW}База пуста.{Style.RESET_ALL}")
    ...
    # Build notes
    lines = [f"# Player Notes — TM Community ({len(games)} games)\n"]
    lines.append(f"*Auto-generated by tm_game_analyzer.py notes*\n")

    # Sort by MMR
    ranked = []
    for name, st in pstats.items():
        if st["games"] < min_games:
            continue
        mmr = mmr_data.get(name, {}).get("mmr", 1000)
        ranked.append((name, st, mmr))
    ranked.sort(key=lambda x: -x[2])

    # Tier boundaries
    tiers = [
        ("Tier 1: Elite (MMR 1080+)", lambda m: m >= 1080),
        ("Tier 2: Strong (MMR 1040-1080)", lambda m: 1040 <= m < 1080),
        ("Tier 3: Average (MMR 1000-1040)", lambda m: 1000 <= m < 1040),
        ("Tier 4: Below Average (MMR 950-1000)", lambda m: 950 <= m < 1000),
        ("Tier 5: Struggling (MMR <950)", lambda m: m < 950),
    ]

    for tier_name, tier_fn in tiers:
        tier_players = [(n, s, m) for n, s, m in ranked if tier_fn(m)]
        if not tier_players:
            continue

        lines.append(f"\n## {tier_name}\n")

        for name, st, mmr in tier_players:
            g = st["games"]
            wr = st["wins"] / g * 100
            avg_vp = st["total_vp"] / g
            avg_cards = st["total_cards"] / g
            ...
            # Favorite corps
            fav_corps = st["corps"].most_common(3)
            corp_str = ", ".join(f"{c}({n}x)" for c, n in fav_corps)

            # Best VP cards
            best_vp = sorted(st["top_vp_cards"].items(),
                             key=lambda x: -sum(x[1]) / len(x[1]))[:3]
            vp_str = ", ".join(f"{c}(avg {sum(v)/len(v):.0f})"
                               for c, v in best_vp) if best_vp else "—"

            # Nemesis and victim
            nemesis_list = st["opponents"].items()
            nemesis = max(((n, d["losses"]) for n, d in nemesis_list if d["losses"] > 0),
                          key=lambda x: x[1], default=None)
            victim = max(((n, d["wins"]) for n, d in nemesis_list if d["wins"] > 0),
                         key=lambda x: x[1], default=None)

            # Game sizes
            sizes = ", ".join(f"{k}={v}" for k, v in sorted(st["game_sizes"].items()))

            # Milestones/Awards from games
            ms_count = 0
            aw_count = 0
            for game in games:
                for p in game["players"]:
                    if normalize_name(p["name"]) == name:
                        ms_count += len(p.get("milestone_details", []))
                        aw_count += sum(1 for a in p.get("award_details", [])
                                        if "1st" in str(a))

            lines.append(f"### {name} — MMR {mmr}, {wr:.0f}% WR, {g} games")
            lines.append(f"- **Style:** {avg_cards:.0f} cards avg, {avg_gen:.1f} gen, {sizes}")
            lines.append(f"- **Corps:** {corp_str}")
            lines.append(f"- **VP engines:** {vp_str}")
            lines.append(f"- **Milestones:** {ms_count} claimed | **Awards 1st:** {aw_count}")
            if nemesis:
                lines.append(f"- **Nemesis:** {nemesis[0]} ({nemesis[1]} losses)")
            if victim:
                lines.append(f"- **Victim:** {victim[0]} ({victim[1]} wins)")
            lines.append("")

    # Write file
    notes_path = os.path.join(DATA_DIR, "player_notes.md")
    with open(notes_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))

    print(f"{Fore.GREEN}Player notes updated: {notes_path}{Style.RESET_ALL}")
    print(f"  {len(ranked)} players (min {min_games} games)")
...
def main():
    ...
    # notes
    p_notes = sub.add_parser("notes", help="Сгенерировать/обновить player_notes.md")
    p_notes.add_argument("--min-games", type=int, default=5, help="Мин. игр для включения")
