...
if (validVariants.length > 0) {
  console.log('\n=== VALID EXPANSION VARIANTS ===');
  validVariants.forEach((name) => {
    const aliasInfo = getAliasInfo(name);
    const base = canonicalBase(name);
    const aliasTags = TAGS[name] || [];
    ...
    console.log(
      ...
      ' | baseData=' + (inData.has(base) ? 'yes' : 'no') +
      ' | baseTags=' + (inTags.has(base) ? fmtTags(baseTags) : 'none') +
      ' | note=' + aliasInfo.note
    );
  });
}
...
if (remainingAliases.length > 0) {
  console.log('\n=== REMAINING BROKEN ALIASES ===');
  remainingAliases.forEach((name) => {
    const aliasInfo = getAliasInfo(name);
    const base = canonicalBase(name);
    const aliasTags = TAGS[name] || [];
    const baseTags = TAGS[base] || [];
    console.log(
      '  ' + name +
      ' -> ' + base +
      ' | kind=' + (aliasInfo ? aliasInfo.kind : 'generic_variant') +
      ' | alias=' + fmtTags(aliasTags) +
      ' | baseData=' + (inData.has(base) ? 'yes' : 'no') +
      ' | baseTags=' + (inTags.has(base) ? fmtTags(baseTags) : 'none') +
      (aliasInfo ? ' | note=' + aliasInfo.note : '')
    );
  });
}
